  ...
  try {
    ...
    if (hasNVIDIAKey) {
      try {
        ...
        if (
          !scrapedData.companyInfo.industry &&
          enrichedData.companyInfo.industry
        ) {
          llmEnrichments.enrichedFields.push("industry");
        }
        if (
          !scrapedData.positioning?.valueProposition &&
          enrichedData.positioning?.valueProposition
        ) {
        ...
        console.log(
          "✅ AI enrichment complete. Enriched fields:",
          llmEnrichments.enrichedFields,
        );
      } catch (llmError) {
        const errorMsg =
          llmError instanceof Error ? llmError.message : "Unknown error";
        console.warn(
          "⚠️  LLM enrichment failed (scraping continues):",
          errorMsg,
        );
    ...
    if (hasSupabaseConfig) {
      try {
        ...
        if (
          saveError.includes("supabase-setup.sql") ||
          saveError.includes("table") ||
          saveError.includes("column")
        ) {
          console.log(
            "💡 Database schema issue detected - guiding user to setup script",
          );
        }
    ...
    return NextResponse.json({
      ...
      warnings: [
        ...
        ...(!hasNVIDIAKey
          ? ["NVIDIA API key not configured - AI enrichment skipped"]
          : []),
      ].filter(Boolean),
    });
  } catch (error) {
