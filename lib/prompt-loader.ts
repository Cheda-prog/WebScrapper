 ...
 */
export function buildPromptVariables(kb: KnowledgeBase): Record<string, string> {
  return {
    ...
    products: kb.products?.map(p => `- ${p.name}: ${p.description || 'N/A'}`).join('\n') || 'None listed',
    target_audience: kb.customers?.targetAudience?.join(', ') || 'Not specified',
    customer_needs: kb.customers?.customerNeeds?.join(', ') || 'Not specified',
    testimonials: kb.testimonials?.map(t => `"${t.content}" - ${t.author}`).join('\n') || 'None',
    brand_tone: kb.branding?.toneOfVoice || 'Professional',
    writing_style: kb.branding?.writingStyle || 'Clear and concise',
    products_features: kb.products?.map(p => 
      `${p.name}: ${p.features?.join(', ') || 'N/A'}`
    ).join('\n') || 'None',
 ...
 */
export async function generatePitchFromTemplate(kb: KnowledgeBase): Promise<string> {
 ...
 */
export async function createPersonasFromTemplate(kb: KnowledgeBase): Promise<string> {
 ...
 */
export async function enrichDataFromTemplate(kb: KnowledgeBase): Promise<string> {
  ...
  const variables = {
    ...
    missing_fields: Object.keys(kb).filter(key => {
